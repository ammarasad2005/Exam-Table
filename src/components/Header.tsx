...
interface HeaderProps {
  ...
  /** When true, hides the semester timeline in the header center. */
  hideTimeline?: boolean;
}

export function Header({ children, rightActions, hideTimeline }: HeaderProps) {
  const hasChildren = !!children;

  return (
    <header
      className="sticky top-0 z-50 flex flex-col flex-shrink-0 glass-header-laser"
      style={{ boxShadow: 'var(--shadow-header)' }}
    >
      {/* Row 1: Logo + Timeline + Actions (always present on desktop) */}
      <div className="flex items-center justify-between h-[3.75rem] px-5 md:px-10">
        {/* Left: Logo */}
        ...
        {/* Mobile center: children (timeline hidden on mobile) */}
        {hasChildren && !hideTimeline && (
          <div className="md:hidden flex-1 flex justify-center mx-2 overflow-hidden min-w-0">
            {children}
          </div>
        )}
      ...
      {/* Row 2: Back + Page Title (desktop only, when children present) */}
      {hasChildren && (
        <div className="hidden md:flex items-center px-5 md:px-10 h-[2rem] border-t border-[var(--color-border)] bg-[var(--color-bg-subtle)]/30">
          {children}
        </div>
      )}
    </header>
